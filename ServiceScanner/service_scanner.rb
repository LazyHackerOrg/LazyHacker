#gem install ruby-nmap

require 'nmap/program'
require 'ipaddr'
require 'socket'

require 'nmap/xml'


class ServiceScanner


end

def localIP
  UDPSocket.open {|s| s.connect("64.233.187.99", 1); s.addr.last}
end

def localSubnet
  localIP.split(".")[0..2].push(0).join(".")
end

def localSubnettoScan
  localSubnet[0..-2]+"*"
end

puts localIP
puts localSubnettoScan

#http://www.rubydoc.info/gems/ruby-nmap/Nmap/Task

Nmap::Program.sudo_scan do |nmap|

  #puede ser '192.168.0.*' ... encontrar ip de red
  # Target
  nmap.targets = localSubnettoScan
  #nmap.targets = '127.0.0.1'

  # Debugging
  nmap.verbose = true
  nmap.show_reason = false

  # Port scanning
  nmap.syn_scan = true
  nmap.all = true    ## -A en nmap

  # Service/Version detection
  nmap.service_scan = true
  nmap.os_fingerprint = true
  nmap.version_all = true

  # Script scanning
  # https://nmap.org/book/nse-usage.html#nse-categories
  # nmap.script = "all"
  nmap.script = "discovery"

  # Ports
  nmap.all_ports    # nmap.ports = (0..65535).to_a

  # Firewall/IDS Evasion
  nmap.decoys = ["google.com","yahoo.com","hotmail.com","facebook.com"]

  # Spoofing
  # nmap.spoof_mac = "00:11:22:33:44:55"

  # Timing and Performance
  nmap.min_parallelism = 30
  nmap.max_parallelism = 130

  # Scan outputs
  nmap.output_all = 'scan_result'


end

Nmap::XML.new('./scan_result.xml') do |xml|
  xml.each_host do |host|
    puts "[#{host.ip}]:::#{host.os.matches.first.to_s}" if host.status.to_s.eql? 'up'

    # Print: Port/Protocol      port_status      service_name
    host.each_port do |port|
      puts "  #{port.number}/#{port.protocol}\t#{port.state}\t#{port.service}::#{port.service.extra_info}"
    end
  end
end

#borrar xml al final